...
export class AuthController {
  ...
  async googleAuthRedirect(@Req() req: Request, @Res() res: Response) {
    ...
    return res.redirect(
      `https://fuhushu.onrender.com/login-success?token=${encodeURIComponent(result.access_token)}&name=${encodeURIComponent(googleUser.name)}&email=${encodeURIComponent(googleUser.email)}`
    )
  }
